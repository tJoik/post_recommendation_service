...
Проект реализует FastAPI-сервис, который по id пользователя, времени запроса и лимиту возвращает список рекомендованных постов.
Для пользователей используется разделение на группы A/B-теста: контрольная и тестовая. Для каждой группы загружается отдельная модель.
...
## Как скачать datasets/

Датасеты не хранятся в репозитории из-за размера, они выложены в разделе [Releases](https://github.com/tJoik/post_recommendation_service/releases/tag/v1.0-data). Архива для загрузки называется datasets.zip. Также можно воспользоваться консольными командами ниже.

Команды для загрузки и распаковки архива, выполнять из корня проекта:
...
Архив распаковывается в корень проекта в папку `datasets/` уже внутри datasets.zip, создавать её вручную в корне не нужно.
...
1. `catboost_control.cbm` - модель для контрольной группы
2. `catboost_test.cbm` - модель для тестовой группы

Для тестовой модели дополнительно используются эмбеддинги текстов постов.

## Ноутбуки

В папке `notebooks/` находятся 2 ноутбука:
1. dev_rec_model.ipynb с подготовкой данных и обучением моделей 
2. ab_tests.ipynb с A/B-тестами.
...
Для работы сервиса локально используются подготовленные CSV-файлы и сохранённые модели, без обращения к БД.